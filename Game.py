...
class Button:
    ...
    def update(self):
        ...
        if collider.colliderrect(mouseLoc):
            if mouseState[0]:
                self.mouseDown()
            else:
                if self.state ==True:
...
class Game:
    ...
    def main(self):
        ...
        centerX = self.WINDOWWIDTH/2-buttonWidth/2
        centerY = self.WINDOWHEIGHT/2-buttonHeight/2
        self.startButton = Button(buttonUp,buttonOver,buttonDown,centerX,
                             centerY)
        #####Game Loop#####
        while self.playing:
